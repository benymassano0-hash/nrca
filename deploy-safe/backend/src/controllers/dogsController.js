    ...
    breederCommercialSchemaPromise = (async () => {
      ...
      await pool.query(
        `INSERT INTO system_settings (setting_key, setting_value, description)
         VALUES ('dog_registration_fee_kz', '10000', 'Taxa base para registo de cão (Kz)')
...
const parseSettingAsNumber = (value, fallbackValue) => {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : fallbackValue;
};

const getDogRegistrationFee = async (dbClient) => {
  const [promoEnabledResult, promoFeeResult, defaultFeeResult] = await Promise.all([
    dbClient.query(
      `SELECT setting_value FROM system_settings WHERE setting_key = $1 LIMIT 1`,
      ['promo_prices_enabled']
    ),
    dbClient.query(
      `SELECT setting_value FROM system_settings WHERE setting_key = $1 LIMIT 1`,
      ['promo_fee_dog_kz']
    ...
    dbClient.query(
      `SELECT setting_value FROM system_settings WHERE setting_key = $1 LIMIT 1`,
      ['dog_registration_fee_kz']
    ),
  ]);

  const defaultFee = parseSettingAsNumber(defaultFeeResult.rows[0]?.setting_value, 10000);
  const promoEnabled = String(promoEnabledResult.rows[0]?.setting_value || '').toLowerCase() === 'true';
  const promoFee = parseSettingAsNumber(promoFeeResult.rows[0]?.setting_value, defaultFee);

  return promoEnabled ? promoFee : defaultFee;
};
  ...
  try {
    ...
    try {
      ...
      const breederAccessResult = await client.query(
        `SELECT id, COALESCE(dog_limit, 0) as dog_limit, COALESCE(balance_kz, 0) as balance_kz
         FROM users
         WHERE id = $1
         LIMIT 1`,
        [breeder.id]
      ...
      if (limitValue > 0 && currentCount >= limitValue) {
        ...
        return res.status(403).json({
          error: `Este criador já atingiu o limite permitido (${limitValue}) de registos de cães.`,
        });
      }

      feeKz = getDogRegistrationFee ? await getDogRegistrationFee(client) : 10000;
      ...
      const debitResult = await client.query(
        `UPDATE users
         ...
         WHERE id = $2
           AND COALESCE(balance_kz, 0) >= $1
         RETURNING balance_kz`,
        [feeKz, breeder.id]
      );
      ...
      if (!debitResult.rows.length) {
        await client.query('ROLLBACK');
        return res.status(400).json({
          error: `Saldo insuficiente. O criador precisa de ${Number(feeKz).toLocaleString('pt-PT')} Kz para registar este cão.`,
        });
      }
      ...
      await client.query(
        ...
        [breeder.id, feeKz, `Débito automático por registo do cão ${registration_id}`, req.user?.id || null]
      );
    ...
    res.status(201).json({
      ...createdDog,
      registration_fee_kz: feeKz,
      registration_fee_label: `${Number(feeKz).toLocaleString('pt-PT')} Kz`,
      message: `Cão registado com sucesso. Taxa debitada automaticamente: ${Number(feeKz).toLocaleString('pt-PT')} Kz.`
    });
  } catch (error) {
    console.error(error);
