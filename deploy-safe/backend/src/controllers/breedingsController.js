...
const getLitterRegistrationFee = async (dbClient) => {
...
const createBreeding = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      feeKz = await getLitterRegistrationFee(client);
      ...
      const debitResult = await client.query(
        ...
        [feeKz, breedingBreederId]
      );

      if (!debitResult.rows.length) {
        await client.query('ROLLBACK');
        return res.status(400).json({
          error: `Saldo insuficiente. O criador precisa de ${Number(feeKz).toLocaleString('pt-PT')} Kz para registar a ninhada.`,
        });
      }
      ...
      await client.query(
        ...
        [breedingBreederId, feeKz, `Débito automático por registo de ninhada`, req.user?.id || null]
      );
    ...
    res.status(201).json({
      ...
      message: `Cruzamento registado com sucesso. Taxa debitada automaticamente: ${Number(feeKz).toLocaleString('pt-PT')} Kz.`
    });
  } catch (error) {
    console.error(error);
